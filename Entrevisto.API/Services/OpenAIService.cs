...
namespace Entrevisto.API.Services
{
    public class OpenAIService : IOpenAIService
    {
        ...
        public async Task<GenerateScriptResponse> GenerateScript(GenerateScriptRequest request)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                throw new Exception($"Error calling OpenAI API: {error}");
            }
        ...
        private (string, string) GetPrompts(GenerateScriptRequest request)
        {
            string systemPrompt;
            string userPrompt;

            if (string.IsNullOrEmpty(request.RegenerationType))
            {
            ...
            else if (request.RegenerationType == "technical")
            {
            ...
            else if (request.RegenerationType == "behavioral")
            {
